        ...
        for keep in mimic_files[samples].keys():
            ...
            if nn_curve:
                ...
                for y in graph_ys:
                    ...
                    p.savefig(
                        '{}_MIMIC_{}_{}_{}.png'.format(problem_name, samples, keep, y.capitalize()),
                        format='png', dpi=150)
        ...
        for i, mate in enumerate(ga_files[pop].keys()):
            ...
            if nn_curve:
                ...
                for y in graph_ys:
                    ...
                    p.savefig(
                        '{}_GA_{}_{}_{}.png'.format(problem_name, pop, mate, y.capitalize()),
                        format='png', dpi=150)
        if nn_curve:
            p.savefig(
                '{}_GA_{}_{}.png'.format(problem_name, pop, 'Score'),
                format='png', dpi=150)
...
def plot_sa_data(problem_name, sa_files, output_dir, nn_curve=False):
    ...
    if nn_curve:
        ...
        for y in graph_ys:
            ...
            p.savefig(
                '{}_SA_{}.png'.format(problem_name, y.capitalize()),
                format='png', dpi=150)
...
def plot_rhc_data(problem_name, rhc_files, output_dir, nn_curve=False):
    ...
    if nn_curve:
        ...
        p.savefig(
            '{}_RHC_{}.png'.format(problem_name, 'Fitness'),
            format='png', dpi=150)
    else:
        for y in graph_ys:
            ...
            p.savefig(
                '{}_RHC_{}.png'.format(problem_name, y.capitalize()),
                format='png', dpi=150)
...
def plot_best_curves(problem_name, files, output_dir, nn_curve=False):
    ...
    p = plot_data('{} - Best: {} vs Iterations'.format(problem_name, y_label), main_df,
                  ...
                  y_label=y_label)
    p.savefig(
        'Best_{}.png'.format(problem_name, 'Fitness'),
        format='png', dpi=150)
...
def read_and_plot_test_output(base_dir, output_dir, problem_name, multiple_trials=True, nn_curve=False):
    ...
    output_file_name_regex = re.compile('{}_([A-Za-z]+)(.*)_LOG\.csv'.format(problem_name))
    output_files = glob.glob('{}_*_LOG.csv'.format(problem_name))
    ...
    for output_file in output_files:
        ...
        if best_value > files[algo]['best']:
            if nn_curve:
                ...
                similar_files = glob.glob('{}_{}{}*_LOG.csv'.format( problem_name, algo, params))
                ...
                curr_max = np.max(np.mean(fitness_data, axis=0))
...
if __name__ == '__main__':
    for problem_name in to_process:
        ...
        best_files = find_best_results('{}'.format(problem['path']), problem_name,
    ...
    with open('best_results.csv', 'w+') as f:
        ...
        for problem_name in sorted(the_best):
            ...
            for algo in best_files:
                ...
                _f, params = output_file_name_regex.search(file).groups()
                if nn_curve:
                    params = list(filter(None, params.split('_')))
                else:
